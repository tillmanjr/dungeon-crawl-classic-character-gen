...
function formatCharacterAsHtml(character, container_id = '') {
  ...
  const id_name_value_attr = container_id ? ` id="name_value_${container_id}" ` : ' '
  const id_name_input_attr = container_id ? ` id="name_input_${container_id}" ` : ' '
  const name_value_js = ` onclick="handleNameValueClick('name_value_${container_id}', 'name_input_${container_id}')" `
  const name_input_js = ` onblur="handleNameEditorBlur('name_value_${container_id}', 'name_input_${container_id}')" `
    const resultArray = []
    resultArray.push(`<div class="characterContainer" ${id_attr}>`)
      resultArray.push('<div class="characterHeader">')
    resultArray.push('<span class="nameLabel">Name:</span>') 
      resultArray.push(`<span ${name_value_js} class="nameValue display-inline-block" ${id_name_value_attr} >${character.Name}</span>`)
      resultArray.push(`<input type="text"  ${name_input_js} class="nameInput display-none" ${id_name_input_attr} value=${character.Name} ></input><span class="raceLabel">Race:</span><span class="raceValue">${character.Race}</span>`)
